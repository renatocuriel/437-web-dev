...
// import loadingSearchResults from "../assets/loadingSearchResults.json"; // Mock search results for now
...
//       setSearchResults(data.books || []); // Ensure it handles missing data gracefully
//     } catch (error) {
...
//           loadingSearchResults.map((book, index) => (
...
//                 <p className="text-sm text-center text-gray-600 dark:text-gray-300">{book.authors[0]}</p>
...
import booksMockData from "../assets/mockBooks.json"; // Mock recommended books
...
const SuggestedBooksPage = () => {
  ...
  const handleSetCurrentlyReading = async (book) => {
    try {
      ...
      setCurrentlyReadingBook(savedBook);
      alert(`Started reading "${book.title}".`);
    } catch (error) {
  ...
  return (
    <div className="mt-6 md:mt-0">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {isLoading ? (
          <p className="text-center text-gray-500 dark:text-gray-400 col-span-full">Loading results...</p>
        ) : searchResults.length > 0 ? (
          searchResults.map((book, index) => (
            <div key={index} className="bg-container p-4 rounded-lg shadow-md border-highlight hover:scale-105 transition-transform flex flex-col h-full">
